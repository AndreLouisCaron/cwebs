 ...
 * FALSE if not.
 */
#define DUP2CLOSE(oldfd, newfd) \
    (dup2(oldfd, newfd) == 0  &&  close(oldfd) == 0)

/*!
 * @brief Replace standard streams with @a rpipe and @a wpipe.
 * @param rpipe Pipe in read-only mode.
 * @param wpipe Pipe in write-only mode.
 *
 * This function closes the current standard input input and output streams and
 * replaces the file descriptors with those referred to by @a rpipe and @a
 * wpipe.  It manages all corner cases, including those where @a rpipe and/or
 * @a wpipe are assigned file descriptors 0 or 1.
 */
